...
(`name`), a longer `description`, a list of `tags`. The parameter
`replace` set to T allows to overwrite the existing item created
before.
...
variable, `myiris` is necessary. (Can be exploited by `build` and to
