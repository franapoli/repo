...
* Load into R session any stored object by name
* Store object description, tags, provenance trace, source code
* Search items by names, tags or text in the description
* Simple and easy versioning
* Summarize repository-wise or item-wise details
* Build and plot item dependency trees
* Attach regular files to repo items
* Run system commands on stored attachments
* Quickly store cache/temp data
* Run expression with automatic caching of results
* Safe concurrent accesses to the same repository
...
the full path of the file. For this guide, however, it will be
avoided:
...
repositories. The variable `repo` will be used as the main interface
...
in the repo. The data will be actually saved in the repo root in RDS
format.
...
The call provides the data to be stored (`obj`), an identifier (`name`), a
longer *description*, a list of *tags*, the name of the *source* file that
generated the data.
...
repo$attach(fpath, "Iris 2D visualization obtained with PCA.",
            c("visualization", "iris", "repodemo"), src, to="myiris")
```
...
commnad. On a Linux system, this command runs the Evince document
...
repo$attach(fpath, "Variance explained by the PCs of the Iris dataset",
            c("visualization", "iris", "repodemo"), src, to="iris2D.pdf")
```
...
```{r}
fpath <- file.path(repo$root(), "iris2Dclu.pdf")
pdf(fpath)
plot(iris2d, main="Iris dataset kmeans clustering", col=kiris)
invisible(dev.off())
...
check the presence of extraneous (not indexed) files.
...
* Add a new version of an existing object (`repo$put(..., replace="addversion")`)
* Stash an object as temporary (`repo$stash`)
* Download remote item contents (`repo$pull`)
...
the precomputed variable from the repo.
...
It is also possible to automate the process of stashing data for cache
...
containing all items information but not the actual data.

```{r}
repo$put("Local content", "item1",
