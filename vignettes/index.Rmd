...
What follows is a walk-through aimed at quickly introducing to the
main features of Repo. Additional information can be found in the
paper: Napolitano, F. *repo: an R package for data-centered management
of bioinformatic pipelines*. **BMC Bioinformatics** 18, 112 (2017).
...
be annotated as being produced by it. This will be automated through
the `options` command.
...
containing the source code that generates the new item (`src`). All
the parameters but `obj` are optional.
...
separately. The following is s a more compact call to `put`:
...
any file in the repo as is, plus annotations. Two parameters differ
from `put`:
...
Plus, item contents can be dowloaded if an URL is provided with it
(`rp$pull`).
...
names are loaded, which may be usefule for example to exploit auto-completion features of the editor.
